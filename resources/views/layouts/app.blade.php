...
<body class="bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 min-h-screen font-sans">
    ...
    <div class="flex">
        ...
        <aside id="sidebar"
            ...
            <div class="flex items-center justify-between p-6 border-b border-gray-100">
                <div class="flex items-center space-x-3" id="logo-section">
                    ...
                    <div class="sidebar-text">
                        <h1 class="text-xl font-bold text-gray-800">Sisepta</h1>
                    ...
                    <ul class="space-y-2">
                        <li>
                            <a href="{{ route('presensi-guru') }}"
                                ...
                                <i class="fas fa-clock text-lg w-5 flex-shrink-0 {{ iconColor(['presensi-guru', 'face-detection-page']) }}"></i>
                                <span class="sidebar-text font-medium">Presensi</span>
                            </a>
                        </li>
                        <li>
                            <a href="{{ route('riwayat-presensi-guru') }}"
                                class="menu-item flex items-center space-x-3 px-4 py-3 rounded-xl {{ menuActive('riwayat') }}">
                                <i class="fas fa-list-check text-lg w-5 flex-shrink-0 {{ iconColor('riwayat') }}"></i>
            ...
            <!-- Sidebar Footer -->
            <div class="p-4 border-t border-gray-100 w-full absolute bottom-0">
                <a href="{{ route('logout') }}"
                ...
                <div class="mb-8">
                    ...
                    <ul class="space-y-2">
                        ...
                        <li>
                            <a href="{{ route('riwayat-presensi-guru') }}"
                                class="menu-item flex items-center space-x-3 px-4 py-3 rounded-xl {{ menuActive('riwayat') }}">
                                <i class="fas fa-list-check text-lg w-5 {{ iconColor('riwayat') }}"></i>
                ...
                <div class="flex items-center justify-between px-6 py-4">
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <div class="hidden md:flex items-center space-x-2 text-sm">
                            <a href="{{ route('dashboard-guru') }}" class="text-gray-500 hover:text-primary-600 transition-colors">Home</a>

                            @foreach (breadcrumb() as $item)
                                <i class="fas fa-chevron-right text-gray-300 text-xs"></i>
                                @if (isset($item['route']))
                                    <a href="{{ route($item['route']) }}" class="text-gray-500 hover:text-primary-600 transition-colors">
                                        {{ $item['label'] }}
                                    </a>
                                @else
                        ...
                        <div class="relative">
                            <button id="user-menu-btn"
                                ...
                                <div class="hidden md:block text-left">
                                    <p class="text-sm font-semibold text-gray-800">{{ \Illuminate\Support\Str::limit(Auth::user()->name, 15) }}</p>
                                ...
                                <div class="p-4 bg-gradient-to-r from-primary-500 to-primary-600 rounded-t-xl">
                                    <div class="flex items-center space-x-3">
                                        <div
                                            ...
                                            <span class="text-white font-semibold text-lg">{{ strtoupper(substr(Auth::user()->name, 0, 2)) }}</span>
                                        </div>
                                        <div>
                                            <p class="text-white font-semibold">{{ \Illuminate\Support\Str::limit(Auth::user()->name, 28) }}</p>
                                            <p class="text-primary-100 text-sm">{{ \Illuminate\Support\Str::limit(Auth::user()->email, 28) }} </p>
                                        </div>
                                    </div>
                                </div>
